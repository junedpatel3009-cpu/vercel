...
/// The client's single project workspace: every job they posted plus the
/// status of any professional hired on it. This is the only project list
/// clients navigate to — there is no separate "Jobs" page for this role.
...
class _ClientProjectsScreenState extends State<ClientProjectsScreen> {
  ...
  Future<void> _loadProjects() async {
    try {
      ...
      try {
        projects = await ApiClient.instance.getList('/api/v1/client/project-board');
      } on ApiException {
        // The project-board enrichment can be unavailable before a hire exists.
        projects = await ApiClient.instance.getList('/api/v1/client/jobs');
      }
  ...
  List<Map<String, dynamic>> get _visibleProjects {
    return _projects.where((job) {
      ...
      return switch (_filter) {
        'Running' => job['project'] is Map && ((job['project'] as Map)['status'] ?? '').toString().toUpperCase() == 'ACTIVE',
        'Hiring' => job['project'] == null && (job['status'] ?? '').toString().toUpperCase() == 'OPEN',
        'Completed' => job['project'] is Map && ((job['project'] as Map)['status'] ?? '').toString().toUpperCase() != 'ACTIVE',
        _ => true,
      };
    }).toList();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Projects', style: TextStyle(color: AppTheme.brandNavy, fontWeight: FontWeight.w800)),
      ...
      body: _loading ? const Center(child: CircularProgressIndicator()) : _buildBoard(),
    );
  }
  ...
  Widget _buildBoard() {
    ...
    final completed = _projects.where((job) => job['project'] is Map && ((job['project'] as Map)['status'] ?? '').toString().toUpperCase() != 'ACTIVE').length;
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          if (_visibleProjects.isEmpty) _emptyBoard() else ..._visibleProjects.asMap().entries.map((entry) => FadeSlideIn(delay: Duration(milliseconds: entry.key * 55), child: _projectCard(entry.value))),
        ],
      ),
    );
  }
