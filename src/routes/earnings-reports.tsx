...
function EarningsReports() {
  ...
  function printReport(title: string, columns: string[], dataRows: any[]) {
    ...
    printWindow.document.write(
      `<!doctype html><html><head><title>${title}</title><style>body{font-family:Arial,sans-serif;padding:24px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px;font-size:12px}th{background:#f8fafc}</style></head><body><h1>${title}</h1><p>Generated on ${new Date().toLocaleString()}</p><table><thead><tr>${columns.map((column) => `<th>${column}</th>`).join("")}</tr></thead><tbody>${reportRows.map((row) => `<tr><td>${row.replace(/\|/g, "</td><td>")}</td></tr>`).join("")}</tbody></table></body></html>`,
    );
        ...
        <AdminSection
          ...
          actions={
            <Button
              ...
              variant="outline"
              size="sm"
              className="rounded-lg h-9 font-bold text-[10px] uppercase tracking-widest"
              onClick={() => {
                setReportPeriod("ALL");
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex gap-1">
                  <Button
                    ...
                    onClick={() => exportReport("ProjectWithdrawal", "CSV")}
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex gap-1">
                  <Button
                    ...
                    variant="outline"
                    className="h-10 rounded-xl"
                    onClick={() => exportReport("ProjectTransaction", "EXCEL")}
                    disabled={!!downloading}
                  >
                    {downloading === "EXCEL" ? (
                      <Loader2 className="h-3 w-3 animate-spin" />
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <div className="rounded-3xl border border-indigo-200 bg-indigo-50/20 p-8 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-indigo-700 font-medium">
              Platform fund movement distribution.
            </p>
          </div>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          ...
          <PipelineMetric
            ...
            value={report.totals.pendingPayouts}
            variant="warning"
          />
          <PipelineMetric
            label="Processing"
            value={report.totals.processingPayouts}
            variant="primary"
          />
          <PipelineMetric
            label="Rejected"
            value={report.totals.rejectedPayouts}
            variant="destructive"
          />
        </div>
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2
              className={cn("text-xl font-bold", isBalanced ? "text-emerald-900" : "text-rose-900")}
            >
        ...
        <div className="mt-8 space-y-4 flex-1">
          <ReconciliationRow
            ...
            value={report.totals.netEarnings}
            variant={isBalanced ? "emerald" : "rose"}
          />
          <ReconciliationRow
            label="Total Reconciled Funds"
            value={reconciledNet}
            variant={isBalanced ? "emerald" : "rose"}
          />
...
function PipelineMetric({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border p-4 shadow-sm bg-white hover:scale-[1.02] transition-transform",
        styles[variant],
      )}
    >
...
function ReconciliationRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4 text-sm px-2">
      <span
        className={cn("font-medium", variant === "emerald" ? "text-emerald-700" : "text-rose-700")}
      >
        {label}
      </span>
      <span
            ...
            <div key={p.professionalId} className="group p-6 hover:bg-muted/30 transition-colors">
              <div className="flex flex-col sm:flex-row justify-between gap-4 mb-6">
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  <Badge
                    variant="outline"
                    className="rounded-lg h-7 font-bold uppercase tracking-widest text-[9px]"
                  >
                  ...
                  {(p.pendingPayouts > 0 || p.availableBalance > 0) && (
                    <Badge
                      variant="secondary"
                      className="rounded-lg h-7 font-bold uppercase tracking-widest text-[9px] bg-amber-50 text-amber-700 border-amber-200"
                    >
                      Action Required
                    </Badge>
                  )}
          ...
          <div className="flex flex-col lg:flex-row justify-between gap-8">
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Badge
                  variant="outline"
                  className="rounded-lg h-7 font-bold uppercase tracking-widest text-[9px] bg-background"
                >
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2 rounded-2xl bg-muted/40 p-4 border border-border/50">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm italic text-muted-foreground">
                    {payout.note || "No administrative notes added."}
                  </p>
                </div>
              </div>
            ...
            <div className="w-full lg:w-72 space-y-4">
              <div className="rounded-2xl border-2 border-primary/20 bg-primary/[0.02] p-5 text-center shadow-sm">
                ...
                <p className="mt-1 text-3xl font-bold text-primary tabular-nums tracking-tight">
              ...
              <div className="space-y-1.5">
                ...
                {pendingPayoutId === payout.id && (
                  <p className="text-center text-[10px] font-bold text-primary animate-pulse uppercase tracking-widest mt-2">
                    Updating Status...
                  </p>
                )}
              </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Badge
                  ...
                  className="rounded-lg uppercase tracking-widest text-[9px]"
                >
                  {formatEnum(t.status)}
                </Badge>
                <Badge
                  variant="outline"
                  className="rounded-lg h-6 font-bold uppercase tracking-widest text-[8px] bg-background"
                >
              ...
              <div className="mt-4 flex flex-wrap gap-8 text-sm">
                <p>
                  ...
                  <span className="font-bold text-foreground">{t.clientName}</span>
                </p>
                <p>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground block mb-1">
                    Professional
                  </span>{" "}
                  <span className="font-bold text-foreground">{t.professionalName}</span>
                </p>
              </div>
            </div>
